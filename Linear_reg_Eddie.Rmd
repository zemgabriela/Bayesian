...
Let us check the results for the best model

```{r}
best_linear_model <- lm(formula = log(median_house_value) ~ median_income + ocean_proximity_INLAND + 
    total_bedrooms + population + housing_median_age + total_rooms + 
    households + longitude + latitude + ocean_proximity_NEAR.BAY + 
    ocean_proximity_ISLAND + ocean_proximity_.1H.OCEAN, data = housing)
